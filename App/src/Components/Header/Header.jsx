...
const Header = () => {
  const { openNav, setOpenNav, theme, setChatMode, reactNavigator } = useContext(ChatContext);
    ...
    <section className={`${theme ? 'Header' : 'Header-dark'}`} style={{ display: openNav ? 'flex' : 'none' }}>
      <div className="top-header" id='full-header'>
        <ul className="top-header-ul">
          <li className="header-li"><FaRegUserCircle onClick={()=>reactNavigator("/profile")} size={40} className='nav-item' color='white' /></li>
