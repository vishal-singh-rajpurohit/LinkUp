                    ...
                    <div className="contact-grid">
                        {
                            searchResut.length <= 0 ?
                                ...
                                searchResut.map((item, index) => (
                                    <SearchContactItem isExistingInContact={item.already_in_contact} conId={item._id} selectChat={selectedTalks} userName={item.userName} index={index} avatar={"avatar"} lastMessage={item.lastMessage || false} messageTimer={"12: 55"} totalNewMessages={2} />
                                ))
                        }
                    </div>}
            </section>
