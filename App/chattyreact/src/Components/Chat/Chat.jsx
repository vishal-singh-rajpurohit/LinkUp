...
const Chat = () => {
  const { setOpenNav, theme, openProfile, userContacts, searchResut, isSearching, setOpenProfile, chatMode, navigator, user } = useContext(ChatContext);
              ...
              <div className="contact-grid">
                {
                  searchResut.length <= 0 ?
                    ...
                    searchResut.map((item, index) => (
                      <SearchContactItem isExistingInContact={item.already_in_contact} userName={item.userName} userId={item._id} index={index} avatar={"avatar"} lastMessage={"Hii i am here"} messageTimer={"12: 55"} totalNewMessages={2} />
                    ))
                }
              </div>
          }
              ...
              <div className="message-top" style={{ display: openProfile ? 'none' : 'flex' }}>
                ...
                <div className="message-top-right">
                  <ul className="message-top-icons">
                    ...
                    <li className="message-top-li-icons"><MdMissedVideoCall size={35} /></li>
                  </ul>
