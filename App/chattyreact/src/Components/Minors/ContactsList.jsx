    ...
    return <>
        {
            contactsList.length !== 0 ? contactsList.map((item, index) => (
                <ContactItems kay={index} selectChat={selectedTalks} conId={item.contact_det._id} userName={item.contact_det.groupName} index={index} avatar={"avatar"} lastMessage={"Hii i am here"} messageTimer={"12: 55"} totalNewMessages={2} />
            )) : <NoUsers />
        }
    </>
}
...
const CreateGroupChatModal = ({ closeModal, setCloseModal }) => {
    ...
    useEffect(() => {
        ...
        // console.log("search reasult: ",)
    }, [searchWord])

    return (
        <section className="create-group-chat-section" style={{ display: closeModal ? 'flex' : 'none' }}>
            <div className="create-chat-modal" id='create-chat-modal-' onClick={close}>
                close
            </div>
            <div className="create-chat-modal" onClick={createGroup}>
                create
            </div>
            <div className="create-group-caht-section">
                <div className="create-group-caht-top">
                    <h3 className="group-chat-heading">Group Chat</h3>
                </div>
                <div className="create-group-caht-top">
                ...
                <div className="create-group-chat-contacts-">
                    ...
                    <div className="contact-grid create-contact-grid">
                        {
                            searchContactList ?
